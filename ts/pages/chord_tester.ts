...
{
    const qTones = (<HTMLInputElement>document.getElementById("checkboxQuarterTonesChordTester")).checked;
    ...
    if (nbKeys > 0)
    {
        for (const [tonicValueCur, scaleIdCur] of keys)
        {
            ...
            const scaleValues: Array<number> = getScaleValues(scaleIdCur);
            const scaleNotesValuesCur: Array<number> = getScaleNotesValues(tonicValueCur, scaleValues);
...
function get2KeysNotesText(scaleNotesValuesArray: Array<Array<number>>, qTones = false): string
{
