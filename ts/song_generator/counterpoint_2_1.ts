...
{ 
    ...
    {
        track = generateCounterpointTrack21Candidate(tonic, scaleValues, nbBars, octave, qNote,
...
function acceptNoteInCounterpoint21(note2Value: number, tonicValue: number, barIndex: number, nbBars: number,
    track21: MidiTrack, track11: MidiTrack, trackCF: (MidiTrack | null) = null): boolean
{
    const track11NbNotes = track11.GetNbNotes();
    const track21NbNotes = track21.GetNbNotes();
    const hasTrackCF = (trackCF != null && trackCF.Events != null && trackCF.Events.length > 1);
    ...
    else if (barIndex < track11NbNotes - 2)
    {
        ...
        let has3ConsecutiveSameNotes: Boolean = (note1Value == note1ValueNext && note2Value == note1ValueNext);
...
function ReduceTrack21(track21: MidiTrack, channelId: number): MidiTrack
{
