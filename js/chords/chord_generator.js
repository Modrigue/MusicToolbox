...
let cRange = 0;
let cEmptyStrings = 4;
function getNotesPositionsOnString(noteValue, stringValue, posMin, posMax, includeEmptyString = false, qTones = false) {
    let positions = [];
...
function generateChords(notesValues, nbStrings = 99, includeEmptyStrings = false, noteBass = -1, qTones = false) {
    ...
    for (let startString = 0; startString <= nbStringsTotal - nbNotes; startString++) {
        ...
        for (let p0 of positionsString0) {
            ...
            // start algorithm
            addChordNoteOnString(notesValues, noteBass, startString, startString + 1, startPositions, chordsPositions, tuningValues, nbStrings, includeEmptyStrings, qTones, hitChordsStrings);
        }
    }
    // sort positions
    ...
    for (let prop of propertiesArray) {
        // filter duplicates
        if (!containsArray(sortedChordsPositions, prop.positions))
...
function addChordNoteOnString(notesValues, bass, startIndex, stringIndex, positionsCur, chordsPositions, tuningValues, nbStrings = 99, includeEmptyStrings = false, qTones = false, hitStrings = [], chordAddedArray = []) {
    // secure
    const nbStringsTotal = tuningValues.length;
    ...
    for (let noteValue of notesValues) {
        ...
        for (let pos of positionsOnString) {
            ...
            if (continueSearch)
                addChordNoteOnString(notesValues, bass, startIndex, stringIndex + 1, positionsCandidate, chordsPositions, tuningValues, nbStrings, includeEmptyStrings, qTones, hitStrings, chordAddedArrayCurrent);
...
function updateFoundChordElements() {
    ...
    for (let indexNote = 0; indexNote < 7; indexNote++) {
        const noteSelector = document.getElementById(`chord_explorer_note${indexNote}`);
        ...
        const intervalLabel = document.getElementById(`chord_explorer_interval${indexNote}`);
...
function updateGeneratedChordsOnFretboard(showBarres = true, includeEmptyStrings = false, showQTones = false, showIntervals = false) {
    ...
    const positionsArray = generateChords(chordNotesValues, nbStringsSelected, includeEmptyStrings, noteBass, showQTones);
