...
function acceptChordInChordsProg(chordsValues, tonicValue, barIndex, nbBars, track, intervalRange) {
    if (containsDuplicates(chordsValues))
        return false;
    return true;
}
function addChordAsArpeggios(track, chordValues, startPosition, duration, nbNotesPerBar) {
...
function acceptNoteInChordsProgBass(noteValue, tonicValue, barIndex, nbBars, track, intervalRange) {
    if (noteValue < 0)
        return false;
...
function hasChordsProgTrackCharNotes(track, tonic, scaleValues) {
...
//# sourceMappingURL=chord_progression.js.map